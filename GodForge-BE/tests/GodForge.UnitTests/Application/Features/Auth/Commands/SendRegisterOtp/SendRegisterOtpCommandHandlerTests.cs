...
public class SendRegisterOtpCommandHandlerTests
{
    ...
    public async Task Handle_GivenNewEmail_PersistsHashedChallengeAndQueuesEmail()
    {
        ...
        var command = new SendRegisterOtpCommand("newuser@example.com", "correlation-id");
    ...
    [Fact]
    public async Task Handle_GivenExistingEmail_ReturnsGenericAcceptedWithoutQueueingEmail()
    {
        var now = DateTimeOffset.UtcNow;
        var command = new SendRegisterOtpCommand("existing@example.com", "correlation-id");
        var existing = User.Create(command.Email, "Existing", "hash", now);
