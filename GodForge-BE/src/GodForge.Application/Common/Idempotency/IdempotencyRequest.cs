...
public static class IdempotencyRequest
{
    ...
    public static ApplicationError? Normalize(string? value, out string? normalized)
    {
        ...
        if (normalized.Length is < 1 or > MaxKeyLength)
            return ApplicationError.Validation("VALIDATION_ERROR", $"Idempotency-Key must contain between 1 and {MaxKeyLength} characters.");
