...
> **Lưu ý về MVP Deployment:** Các worker (Parser, Analyze, Diff, Preview, v.v.) được định nghĩa ở đây là các **thành phần logic (logical workers)**. Trong giai đoạn MVP, chúng có thể được chạy chung bên trong một shared host duy nhất (ví dụ: project `GodForge.Worker`) để tiết kiệm tài nguyên và đơn giản hóa việc deploy. Tuy nhiên, các nguyên tắc về queue riêng biệt, tính idempotency, retry policy, DLQ, metrics, và job lifecycle **vẫn được áp dụng nghiêm ngặt cho từng logical worker** như khi chúng được triển khai thành các vi dịch vụ độc lập.
